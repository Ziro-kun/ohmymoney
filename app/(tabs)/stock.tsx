...
export default function StockScreen() {
  ...
  const { assets, netWorth, loadData, addAsset, updateAsset, deleteAsset } = useFinanceStore();
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]}>
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>자산관리 (Stock)</Text>
        <TouchableOpacity
      ...
      <View style={[styles.netWorthCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
        <Text style={[styles.netWorthLabel, { color: colors.textMuted }]}>현재 순자산</Text>
        <Text style={[styles.netWorthValue, { color: colors.text }]}>
          ₩{formatNumber(netWorth)}
        </Text>
      </View>

      <Text style={[styles.sectionTitle, { color: colors.textMuted }]}>보유 자산 및 부채</Text>
      <FlatList
        data={assets}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={[styles.assetItem, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}
          ...
          >
            ...
            <Text style={[
              ...
              {item.type === "liability" ? "-" : ""}{formatNumber(item.amount)}원
            </Text>
          </TouchableOpacity>
        )}
      />
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
            ...
            <View style={styles.typeSelector}>
              <TouchableOpacity
                style={[styles.typeBtn, type === "asset" && { backgroundColor: colors.accent }]}
                onPress={() => setType("asset")}
              >
                <Text style={[styles.typeBtnText, type === "asset" && { color: "#FFF" }]}>자산</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.typeBtn, type === "liability" && { backgroundColor: colors.danger }]}
                onPress={() => setType("liability")}
              >
                <Text style={[styles.typeBtnText, type === "liability" && { color: "#FFF" }]}>부채</Text>
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  netWorthCard: { margin: 20, padding: 30, borderRadius: 24, alignItems: "center", borderWidth: 1 },
  netWorthLabel: { fontSize: 14, fontWeight: "600", marginBottom: 8 },
  netWorthValue: { fontSize: 32, fontWeight: "800", letterSpacing: -1 },
  sectionTitle: { fontSize: 13, fontWeight: "600", marginLeft: 20, marginBottom: 10 },
  list: { paddingHorizontal: 20, paddingBottom: 40 },
  assetItem: { flexDirection: "row", justifyContent: "space-between", padding: 20, borderRadius: 16, marginBottom: 12, borderWidth: 1, alignItems: "center" },
  assetInfo: { flex: 1 },
